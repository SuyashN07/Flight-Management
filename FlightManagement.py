...
db = c.connect(host = 'localhost', user = 'root', passwd = 'noddy123')
mc = db.cursor()
db.autocommit = True
...
mc.execute("create table if not exists bookings(name varchar(100), mobl varchar(15) primary key, age int, gender varchar(50), class_type varchar(500), fro_f varchar(100), to_f varchar(100), date_d varchar(20), tic_price varchar(20), food_price varchar(20))")
mc.execute("create table if not exists passenger(name varchar(20), mobile varchar(15) primary key, class_t varchar(500), boarding_point varchar(20), destination varchar(20), food_req varchar(20))")
...
print("******************************************")
print("Welcome to Flight reservations management.")
print("******************************************")
print("")
...
def ticket_booking():
    ...
    import csv as cs
    import random as rd
    mycon=mysql.connector.connect(host='localhost',user='root',passwd='noddy123',database='reservation')
    cursor=mycon.cursor()
...
def class_ty(phno):
    ...
    if c_type == 1:
        cursor.execute("update bookings set class_type = 'Economy class' where mobl = %s", (mobli,))
        cursor.execute("update bookings set tic_price = 'Rs.6000/-' where mobl = %s", (mobli,))
        cursor.execute("update passenger set class_t = 'Economy class' where mobile = %s", (mobli,))
        
    elif c_type == 2:
        cursor.execute("update bookings set class_type = 'Premium Economy class' where mobl = %s", (mobli,))
        cursor.execute("update bookings set tic_price = 'Rs.10000/-' where mobl = %s", (mobli,))
        cursor.execute("update passenger set class_t = 'Premium Economy class' where mobile = %s", (mobli,))
        
    elif c_type == 3:
        cursor.execute("update bookings set class_type = 'Business' where mobl = %s", (mobli,))
        cursor.execute("update bookings set tic_price = 'Rs.25500/-' where mobl = %s", (mobli,))
        cursor.execute("update passenger set class_t = 'Business' where mobile = %s", (mobli,))
